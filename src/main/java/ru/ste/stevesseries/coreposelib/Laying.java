...
import ru.ste.stevesseries.corebase.NMSUtil;
...
public class Laying extends Pose {
    ...
    public void renderFor(Player op, Player p) {
        ...
        if(POSED.get(p.getUniqueId()).helmet != null) {
            try {
                //((CraftPlayer) p).getHandle().playerConnection.sendPacket(new PacketPlayOutEntityEquipment((int) entityPlayer.getClass().getMethod("getId").invoke(entityPlayer), EnumItemSlot.HEAD, CraftItemStack.asNMSCopy(p.getEquipment().getHelmet())));
    ...
    public void unrenderFor(Player op, Player p) {
        ...
        //playerConnection.sendPacket(new PacketPlayOutPlayerInfo(PacketPlayOutPlayerInfo.EnumPlayerInfoAction.REMOVE_PLAYER, ep));
    }
